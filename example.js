...
/***/ function(module, exports, __webpack_require__) {
	...
	var doubleNested = Container_1.default({
	    initialState: false,
	    view: function (_a) {
	        var truth = _a.model, update = _a.update;
	        return _1.h('div', ['is: ' + truth]);
	    }
	});
	var nestedContainer = Container_1.default({
	    initialState: 0,
	    view: function (_a) {
	        var _b = _a.modules, undo = _b.undo, redo = _b.redo, update = _a.update, count = _a.model;
	        var add = function (x) { return _1.h('button', {
	            on: { click: update({ by: function (count) { return count + x; } }) }
	        }, [x]); };
	        return _1.h('div', [
	            _1.h('h1', [("count: " + count)]),
	            _1.h('div', [add(-1), add(-2), add(2), add(1)]),
	            _1.h('div', [
	                _1.h('button', { on: { click: update(undo) } }, ['Undo']),
	                _1.h('button', { on: { click: update(redo) } }, ['Redo'])
	            ])
	        ]);
	    }
	});
	var container = Container_1.default({
	    nestedContainers: { nestedContainer: nestedContainer },
	    initialState: 'World',
	    view: function (_a) {
	        var nestedContainer = _a.containers.nestedContainer, _b = _a.modules, undo = _b.undo, redo = _b.redo, name = _a.model, update = _a.update;
	        return _1.h('div', [
	            _1.h('div', [
	                _1.h('label', ['your name:']),
	                _1.h('input', {
	                    on: { input: update({
	                            map: function (event) { return event.target.value; },
	                            by: function (name, targetValue) { return targetValue; }
	                        }) },
	                    props: { value: name }
	                })
	            ]),
	            ...
	            _1.h('div', [
	                _1.h('p', ['the nested container:']),
	                nestedContainer()
	            ])
	        ]);
	    }
	});
	_1.default(container, '#example');
...
/***/ function(module, exports, __webpack_require__) {
	...
	function UndoRedo(undoRedoModel) {
	    return immutable_1.Map(undoRedoModel);
	}
	exports.UndoRedo = UndoRedo;
	function ContainerModel(containerModel) {
	    return immutable_1.Map(containerModel);
	}
	function NestedContainers(nestedContainers) {
	    return immutable_1.Map(nestedContainers);
	}
	function Container(container) {
	    var initialState = container.initialState, nestedContainers = container.nestedContainers, view = container.view;
	    var updates$ = xstream_1.default.never();
	    var sendUpdate = updates$.shamefullySendNext.bind(updates$);
	    function resolveModel(container) {
	        return ContainerModel({
	            ...
	            nestedContainers: NestedContainers(Object
	                ...
	                .reduce(function (nestedObj, nestedContainerModel) {
	    ...
	    var initialContainerModel = resolveModel(container);
	    var componentState$ = updates$.fold(function (state, update) { return update(state); }, initialContainerModel);
	    function resolveView(props, containerModel, chain) {
	        if (chain === void 0) { chain = []; }
	        var model = containerModel.get('undoRedoModel').get('current');
	        var containers = (containerModel
	            ...
	            .map(function (_a) {
	            var key = _a.key, container = _a.container;
	            return ({
	                key: key,
	                resolvedView: function (props) { return (resolveView(props, container, chain.concat([key]))); }
	            });
	        })
	            .reduce(function (obj, _a) {
	            var key = _a.key, resolvedView = _a.resolvedView;
	            obj[key] = resolvedView;
	            return obj;
	        }, {}));
	        var lookupChain = chain.reduce(function (lookupChain, componentKey) { return lookupChain.concat(['nestedContainers', componentKey]); }, []);
	        var update = function (updater) {
	            var by = updater.by, byInternal = updater.byInternal;
	            var map = updater.map || (function (event) { return event; });
	            if (by && byInternal)
	                throw ('cannot update both internal model and regular model');
	            if (by) {
	                return function (event) { return (sendUpdate(function (containerModel) { return ((containerModel
	                    .updateIn(lookupChain.concat(['undoRedoModel', 'previousStates']), function (previousStates) { return previousStates.concat([
	                    containerModel.getIn(lookupChain.concat(['undoRedoModel', 'current']))
	                ]); })
	                    .setIn(lookupChain.concat(['undoRedoModel', 'nextStates']), [])
	                    .updateIn(lookupChain.concat(['undoRedoModel', 'current']), function (model) { return by(model, map(event)); }))); })); };
	            }
	            return function (event) { return (sendUpdate(function (containerModel) { return (containerModel.updateIn(lookupChain.slice(), function (m) { return byInternal(m, map(event)); })); })); };
	        };
	        var undo = {
	            map: function (event) { return event; },
	            byInternal: function (model) {
	                var previousStates = model.getIn(['undoRedoModel', 'previousStates']);
	                if (previousStates.length === 0)
	                    return model;
	                return (model
	                    .updateIn(['undoRedoModel', 'nextStates'], function (nextStates) { return nextStates.concat([
	                    model.getIn(['undoRedoModel', 'current'])
	                ]); })
	                    .setIn(['undoRedoModel', 'current'], previousStates[previousStates.length - 1])
	                    .updateIn(['undoRedoModel', 'previousStates'], function (previousStates) { return previousStates.slice(0, previousStates.length - 1); }));
	            }
	        };
	        var redo = {
	            map: function (event) { return event; },
	            byInternal: function (model) {
	                var nextStates = model.getIn(['undoRedoModel', 'nextStates']);
	                if (nextStates.length === 0)
	                    return model;
	                return (model
	                    .updateIn(['undoRedoModel', 'previousStates'], function (previousStates) { return previousStates.concat([
	                    model.getIn(['undoRedoModel', 'current'])
	                ]); })
	                    .setIn(['undoRedoModel', 'current'], nextStates[nextStates.length - 1])
	                    .updateIn(['undoRedoModel', 'nextStates'], function (nextStates) { return nextStates.slice(0, nextStates.length - 1); }));
	            }
	        };
	        var modules = { undo: undo, redo: redo };
	        var view = containerModel.get('view');
	        return view({ model: model, containers: containers, update: update, props: props, modules: modules });
	    }
	    var resolvedView = function (containerModel) { return (resolveView(null, containerModel)); };
	    var view$ = componentState$.map(function (state) { return resolvedView(state); });
	    return {
	        componentState$: componentState$,
	        state$: componentState$.map(function (componentState) { return (componentState.getIn(['undoRedoModel', 'current'])); }),
	        view: view,
	        view$: view$,
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.h = __webpack_require__(14);
	...
	function harmonize(container, selector) {
	    var node = document.querySelector(selector);
	    container.view$.addListener({
	        next: function (newNode) {
	            patch(node, newNode);
	            node = newNode;
	        },
...
/***/ function(module, exports, __webpack_require__) {
	...
	var domApi = __webpack_require__(13);
